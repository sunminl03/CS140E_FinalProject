...
static void data_abort_handler(regs_t *r) {
    ...
    if(fault_addr != illegal_addr) {
        ...
        uint32_t dfsr_val = cp15_dfsr_get();
        uint32_t dfsr_status = bits_get(dfsr_val, 0, 3);
        trace("Reason: %b\n", dfsr_status);
        trace("wtf, fault_addr=%x, dfsr_val=%b, dfsr_status=%b, lr=%x\n", fault_addr, dfsr_val, dfsr_status, r->regs[14]);
    }

    mmu_disable();
    // r->regs[14] = r->regs[14] - 4;
    switchto(r);
}
...
void notmain(void) { 
    ...
    //  - will implement in next vm lab.
    // mmu_init();
    uint32_t domain_reg = (DOM_client << dom_kern*2) | (DOM_client << dom_heap*2);
    vm_mmu_init(domain_reg);
    ...
    vm_map_sec(pt, SEG_HEAP, SEG_HEAP, heap); 
    ...
    mmu_enable();
    assert(mmu_is_enabled());
    output("about to GET32 from <%x>\n", illegal_addr);
    // this <PUT32> should cause a data_abort.
    GET32(illegal_addr);

    mmu_enable();
    output("about to PUT32 to <%x>\n", illegal_addr);
    PUT32(illegal_addr, 0xdeadbeef);

    PUT32(illegal_addr, 0xe12fff1e);
    mmu_enable();
    output("about to jump to <%x>\n", illegal_addr);

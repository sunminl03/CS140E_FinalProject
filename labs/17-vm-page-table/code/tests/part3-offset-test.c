...
void notmain(void) { 
    ...
    for (int i = 0; i < OneMB; i ++) {
        uint32_t new_user_va_addr = user_va_addr + i; 
        uint32_t new_user_pa_addr = user_pa_addr + i; 
        vm_map_sec(pt, new_user_va_addr, new_user_pa_addr, usr);
        PUT32(new_user_pa_addr, 0xdeadbeef);
    ...
    // both va_addrs should be mapped to the same pa_addr

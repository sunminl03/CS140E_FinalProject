...
static inline void *ptr_roundup(void *ptr, uint32_t n) {
...
#define safe_cast(_lhs,_rhs) ({                                     \
    typeof(lhs) _lhs = _lhs;                                        \
    typeof(rhs) _rhs = _rhs;                                        \
    if(sizeof(lhs) < sizeof(hrs))                                   \
        panic("sizeof %s is less than sizeof %s: will lose bits\n", \
